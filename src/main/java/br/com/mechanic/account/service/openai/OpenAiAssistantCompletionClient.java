...
/**
 * Fluxo OpenAI Assistants: thread, mensagem do utilizador (JSON do payload do tópico), run e leitura da resposta.
 * O assistente já carrega instruções no painel OpenAI; o {@code systemPrompt} desta porta é ignorado.
 */
public class OpenAiAssistantCompletionClient implements OpenAiChatCompletionPort {

    private final RestClient restClient;
    private final OpenAiProperties properties;
    private final ObjectMapper objectMapper;

    public OpenAiAssistantCompletionClient(OpenAiProperties properties, ObjectMapper objectMapper) {
        this.properties = properties;
        this.objectMapper = objectMapper;
        RestClient.Builder builder = RestClient.builder()
                .baseUrl(properties.getResolvedBaseUrlHost())
                .defaultHeader(OpenAiAssistantsApiConstants.HEADER_OPENAI_BETA, OpenAiAssistantsApiConstants.HEADER_VALUE_ASSISTANTS_V2);
    ...
    private String createRun(String threadId) throws Exception {
        ObjectNode body = objectMapper.createObjectNode();
        body.put(OpenAiAssistantsApiConstants.JSON_BODY_KEY_ASSISTANT_ID, properties.getAssistantId().trim());
    ...
    private void deleteThreadQuietly(String threadId) {
        try {
            ...
            // Limpeza best-effort; não falha a operação principal.
        }
    }
}
